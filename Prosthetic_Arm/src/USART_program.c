...
volatile USART_t * USART_base_array[3] = { USART1_START_ADDRESS,
		USART2_START_ADDRESS, USART3_START_ADDRESS };
...
void USART_voidInit(void) {
	...
	for (Local_u8Counter = 0; Local_u8Counter < 3; Local_u8Counter++) {
		...
		f32 Local_f32FractionTemp = (f32) (USART_configurations[Local_u8Counter].frequency / (f32) USART_configurations[Local_u8Counter].baudrate);
		...
		USART_base_array[Local_u8Counter]->BRR |= Local_u16MantissaTemp
				<< BRR_DIV_MANTISSA;

		Local_f32FractionTemp = (f32) (Local_f32FractionTemp
				- Local_u16MantissaTemp);
		Local_f32FractionTemp = Local_f32FractionTemp * 16.0;

		Local_u16Mask = 0xFFF0;

		USART_base_array[Local_u8Counter]->BRR &= Local_u16Mask;
		USART_base_array[Local_u8Counter]->BRR |= ((u16) Local_f32FractionTemp)
				<< BRR_DIV_FRACTION;
		...
		Local_u32CR1_Buffer |= USART_configurations[Local_u8Counter].ue
				<< CR1_UE;
		Local_u32CR1_Buffer |= USART_configurations[Local_u8Counter].m << CR1_M;
		Local_u32CR1_Buffer |= USART_configurations[Local_u8Counter].pce
				<< CR1_PCE;
		Local_u32CR1_Buffer |= USART_configurations[Local_u8Counter].tcie
				<< CR1_TCIE;
		Local_u32CR1_Buffer |= USART_configurations[Local_u8Counter].rxneie
				<< CR1_RXNEIE;
		Local_u32CR1_Buffer |= USART_configurations[Local_u8Counter].te
				<< CR1_TE;
		Local_u32CR1_Buffer |= USART_configurations[Local_u8Counter].re
				<< CR1_RE;

		Local_u32CR2_Buffer |= USART_configurations[Local_u8Counter].stop
				<< CR2_STOP;

		Local_u32CR3_Buffer |= USART_configurations[Local_u8Counter].dmat
				<< CR3_DMAT;
		Local_u32CR3_Buffer |= USART_configurations[Local_u8Counter].dmar
				<< CR3_DMAR;
...
void USART_voidInitUSART1(void) {
	...
	Local_f32FractionTemp =
			(f32) (Local_f32FractionTemp - Local_u16MantissaTemp);
	Local_f32FractionTemp = Local_f32FractionTemp * 16.0;

	Local_u16Mask = 0xFFF0;

	USART1_START_ADDRESS->BRR &= Local_u16Mask;
	USART1_START_ADDRESS->BRR |= ((u16) Local_f32FractionTemp)
			<< BRR_DIV_FRACTION;
...
u8 USART_u8SendDataSynchronous(u8 Copy_u8SerialPort, u8 Copy_u8Data) {
	...
	switch (Copy_u8SerialPort) {
		...
		case (USART_SERIAL2):
			USART2_START_ADDRESS->DR = Copy_u8Data;
			while (GET_BIT(USART2_START_ADDRESS->SR, SR_TXE) != 1);
			CLR_BIT(USART2_START_ADDRESS->SR, SR_TXE);
			break;

		case (USART_SERIAL3):
			USART3_START_ADDRESS->DR = Copy_u8Data;
			while (GET_BIT(USART3_START_ADDRESS->SR, SR_TXE) != 1);
			CLR_BIT(USART3_START_ADDRESS->SR, SR_TXE);
			break;
		}
...
u8 USART_u8SendArrayOfDataSynchronous(u8 * Pointer_u8Data) {
	...
	while (Pointer_u8Data[Local_u8Counter] != '\0') {
		...
		USART_u8SendDataSynchronous(USART_SERIAL1, Pointer_u8Data[Local_u8Counter]);
		Local_u8Counter++;
...
u8 USART_u8ReceiveDataSynchronous(u8 Copy_u8SerialPort, u8 * Pointer_u8Data) {
	...
	case (USART_SERIAL2):
		while (GET_BIT(USART2_START_ADDRESS->SR, SR_RXNE) != 1);
		*Pointer_u8Data = USART2_START_ADDRESS->DR;
		CLR_BIT(USART2_START_ADDRESS->SR, SR_RXNE);
		break;

	case (USART_SERIAL3):
		while (GET_BIT(USART3_START_ADDRESS->SR, SR_RXNE) != 1);
		*Pointer_u8Data = USART3_START_ADDRESS->DR;
		CLR_BIT(USART3_START_ADDRESS->SR, SR_RXNE);
		break;
	}
...
u8 USART_u8ReceiveDataSynchronousWithTimeout(u8 * Pointer_u8Data,
		u32 Copy_u32TimeuotPeriod) {

	u8 Local_u8ErrorState = OK;
	u32 Local_u32Countdown = Copy_u32TimeuotPeriod;

	while ((GET_BIT(USART1_START_ADDRESS->SR, SR_RXNE) != 1)
			&& Local_u32Countdown != 0) {
		Local_u32Countdown--;
	}
...
u8 USART_u8ReceiveArrayOfDataSynchronousWithTimeout(
		u8 * Pointer_u8ReceiveBuffer, u32 Copy_u32TimeoutPeriod) {
	...
	while (Local_u8NestedErrorState != TIMEOUT_STATE) {
		...
		Local_u8NestedErrorState = USART_u8ReceiveDataSynchronousWithTimeout(
				&(Pointer_u8ReceiveBuffer[Local_u8Counter]),
				Copy_u32TimeoutPeriod);
		Local_u8Counter++;
	}
...
u8 USART_u8ReceiveArrayOfDataSynchronous(u8 * Pointer_u8ReceiveBuffer,
		u8 Copy_u8NumberOfBytes) {

	u8 Local_u8ErrorState = OK;
	u8 Local_u8Counter = 0;

	for (Local_u8Counter = 0; Local_u8Counter < Copy_u8NumberOfBytes;
			Local_u8Counter++) {

		USART_u8ReceiveDataSynchronous(USART_SERIAL1, &(Pointer_u8ReceiveBuffer[Local_u8Counter]));
...
u8 USART_u8ReceiveStringUntil(u8 * Pointer_u8ReceiveBuffer,
		u8 Copy_u8StopCharacter) {

	u8 Local_u8ErrorState = OK;
	u8 Local_u8Counter = 0;

	USART_u8ReceiveDataSynchronous(USART_SERIAL1, &(Pointer_u8ReceiveBuffer[0]));
...
void USART_voidEnableInterrupt(u8 Copy_u8InterruptType) {
	...
	case (USART_RX_BUFFER_NOT_EMPTY_INT):
		SET_BIT(USART1_START_ADDRESS->CR1, CR1_RXNEIE);
		break;
	case (USART_TX_COMPLETE_INT):
		SET_BIT(USART1_START_ADDRESS->CR1, CR1_TCIE);
		break;
	case (USART_TX_BUFFER_EMPTY_INT):
		SET_BIT(USART1_START_ADDRESS->CR1, CR1_TXEIE);
		break;
	}
...
void USART_voidDisableInterrupt(u8 Copy_u8InterruptType) {
	...
	case (USART_TX_COMPLETE_INT):
		...
		break;
	case (USART_TX_BUFFER_EMPTY_INT):
		CLR_BIT(USART1_START_ADDRESS->CR1, CR1_TXEIE);
		break;
	}
